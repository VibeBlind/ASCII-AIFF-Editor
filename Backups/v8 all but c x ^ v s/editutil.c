...
*/
void printSideMenu(soundfile_t *snd, int mark, int canPaste, int isModified){
	int i;
	for(i = 0; i < COLS; i++){
	...
	if(canPaste){ /* has stuff in the buffer */
		mvprintw(11, COLS-20, "  ^: insert before");
		mvprintw(12, COLS-20, "  v: insert after");
		mvprintw(LINES-1, COLS-20, " Buffered: bytes in buf");
	} else {
		mvprintw(11, COLS-20, "                   ");
		mvprintw(12, COLS-20, "                   ");
