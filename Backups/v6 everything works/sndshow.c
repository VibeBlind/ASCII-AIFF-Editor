...
int main(int argc, char *argv[]){
	...
	int max = power(2, snd.bitDepth -1)-1; /* maximum possible value for bit depth */
	
	/* print out the values */
	char *buffer = calloc(9, 1); /* first part is 9 chars long */
	for(i = 0; i < index; i++) { /* put the sample number in the first 9 chars */
		sprintf(buffer, "%d", i);
		...
		printDisplayed(allDisplayed[i], max, w); /* print out the dash bar representation */
